  ...
  const fetchData = async () => {
    try {
      ...
      if (deptsData.length > 0) {
        setNewAsset((prev) => ({ ...prev, department_id: deptsData[0].id.toString() }));
      }
    } catch (error) {
        ...
        <div className="flex-1 flex flex-col bg-surface-container-lowest border border-outline-variant rounded overflow-hidden">
          ...
          <div className="overflow-x-auto flex-1">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className={`text-xs text-on-surface divide-y divide-[#E2E8F0] ${loading ? "opacity-50" : ""}`}>
                ...
                {filteredAssets.length === 0 && (
                  <tr>
                    <td colSpan={8} className="py-8 text-center text-on-surface-variant">
                      {loading ? "Loading assets..." : "No assets matched the active filters."}
                    </td>
                  </tr>
                )}
              </tbody>
