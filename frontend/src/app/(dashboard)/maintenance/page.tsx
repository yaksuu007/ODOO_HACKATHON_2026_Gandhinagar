...
export default function Maintenance() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (assetsData.length > 0) {
        setNewTicket((prev) => ({ ...prev, asset_id: assetsData[0].id }));
      }
    } catch (err) {
  ...
  const handleUpdateStatus = async (ticketId: number, status: string) => {
    try {
      await apiRequest(`/maintenance/${ticketId}?ticket_status=${status}`, {
        method: "PUT"
      });
              ...
              {tickets.length === 0 && (
                <tr>
                  <td colSpan={8} className="py-8 text-center text-on-surface-variant">
                    {loading ? "Loading tickets..." : "No active maintenance tickets."}
                  </td>
                </tr>
              )}
            </tbody>
