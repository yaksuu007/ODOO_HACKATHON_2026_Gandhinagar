...
import { Package, Lock, Mail, AlertCircle, CheckCircle } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiRequest("/auth/register", {
        method: "POST",
        body: JSON.stringify({ username: email, email, password, firstName, lastName, organizationSlug }),
      });
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            ...
            <input
              ...
              className="w-full bg-surface-container-low border border-outline-variant text-on-surface text-sm rounded px-3 py-2.5 focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all placeholder:text-on-surface-variant/40"
            />
          </div>
