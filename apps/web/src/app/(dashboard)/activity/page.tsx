...
import { ActivityRowSkeleton } from "@/components/ui/card-skeleton";
...
export default function ActivityLogs() {
  ...
  const filtered = logs.filter((l) => {
    ...
    return (
      ...
      (l.changes && JSON.stringify(l.changes).toLowerCase().includes(q))
    );
  });
              ...
              {loading ? (
                ...
                <tr key={log.id} className="hover:bg-surface-container-low transition-colors">
                  ...
                  <td className="py-3 px-6">
                    ...
                    {log.entityId && (
                      <span className="ml-1.5 font-mono text-[11px]">{log.entityId}</span>
                    )}
                  </td>
